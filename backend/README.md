...
с access- и refresh-токенами, а также привязка заявок к пользователям (автор `created_by_id`
и многие-ко-многим исполнители через `ticket_assignments`) (миграции `0001`, `0002`, `0003`, `0004`).
...
- **Заявки и система:**
  - `POST /api/v1/tickets` — создать заявку (с возможностью сразу назначить исполнителей `worker_ids` и фиксацией автора);
  - `GET /api/v1/tickets` — список заявок с фильтрами (`status`, `city_id`, `district_id`, `worker_id`, `unassigned`) и пагинацией;
  - `GET /api/v1/tickets/{id}` — получить одну заявку с адресом, координатами, автором и назначенными исполнителями;
  - `PATCH /api/v1/tickets/{id}` — изменить параметры заявки, статус или назначить/снять исполнителей;
  - `POST /api/v1/tickets/{id}/assign` — обновить состав исполнителей заявки;
...
- На одну заявку может быть назначено несколько исполнителей (`ticket_assignments`, Many-to-Many).
- Автор заявки фиксируется в `tickets.created_by_id`.
...
│       └── tickets/
│           ├── models.py      # заявки и связующая таблица ticket_assignments
│           ├── enums.py       # статусы
│           ├── schemas.py     # поля запроса, проверки и ответ
│           ├── router.py      # POST, GET списка / по ID, PATCH, POST /{id}/assign
│           ├── service.py     # создание, чтение, обновление и транзакции
│           └── repository.py  # SQL-запросы заявок и назначений
├── migrations/                # последовательные изменения схемы БД (0001, 0002, 0003, 0004)
...
Всего **26 запросов**. В `02-ticket-list/folder.bru` находятся общие проверки
...
Последняя проверка — список заявок с фильтрами и пагинацией (13.09.2026):
...
Проверка подготовки CI (14.09.2026): все **48 тестов текущей ветки main** прошли
...
тестовый PostgreSQL остановлен. Первый запуск самого workflow на Ubuntu в GitHub
состоится после push; он ещё не выполнялся.
